...
class BaristaManagementController extends Controller
{
    /**
        ...
        try{
            ...
            foreach($data as $transaction_key => $page){
                ...
                foreach($page->transaction_detail as $key => $transaction){
                    if($transaction->product->handle_by != 'BARISTA'){
            ...
            return view('barista.order-list-undelivered')->with('data', $data);
        
        } catch (Exception $e) {
            report($e);

            return redirect('barista-order-undelivered');
        }
    }
